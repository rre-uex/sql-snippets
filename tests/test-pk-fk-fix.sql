-- Test query to verify PK and FK detection for columns that are both
...
table_relations AS (
    SELECT DISTINCT
        ...
        group_concat('    ' || column_type || ' ' || column_name || 
            CASE WHEN is_primary_key > 0 THEN ' PK' ELSE '' END ||
            CASE WHEN EXISTS(
            ...
            ) THEN ' FK' ELSE '' END, char(10)) as columns
    FROM table_info
    GROUP BY table_name
)
SELECT 
...
WHERE table_name IN ('TRABAJA_EN', 'LOCALIZACIONES_DPTO', 'modelos')
ORDER BY table_name;